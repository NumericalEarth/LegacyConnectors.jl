...
#   - avoids the parser and the cross-grid interpolation step,
#   - and reads as the math it is.
#
# Sounding files come into their own when there is no analytic form —
# real radiosondes, GFS point profiles, etc.
...
# canonical "wet" run (surface qᵛ cap at 14 g/kg, tropopause at 12 km).
...
# For simplicity in this example we use a fixed-qᵛ profile rather than
# inverting hydrostatic balance for `RH·qᵛs(T, p)`. That's what the
# generator in `data/soundings/generate_weisman_klemp_1982.jl` does;
# it's straightforward but distracts from the point here.

qᵛ_wk(z) = min(qᵛ_max, qᵛ_max * RH_wk(z))
...
qᵛ_a = CenterField(grid)
u_a  = CenterField(grid)

set!(θ_a,  (x, y, z) -> θ_wk(z))
set!(qᵛ_a, (x, y, z) -> qᵛ_wk(z))
...
qᵛ_s = CenterField(grid)
u_s  = CenterField(grid)

interpolate!(θ_s,  sounding.potential_temperature)
interpolate!(qᵛ_s, sounding.specific_humidity)
interpolate!(u_s,  sounding.x_momentum)
...
lines!(ax_qᵛ, qᵛ_a * 1000;                          linewidth = 2)
lines!(ax_qᵛ, qᵛ_s * 1000;                          linestyle = :dash)
...
# The two paths agree on θ and u to plotting tolerance. The qᵛ panel
# shows a small offset because the bundled `.txt` was generated with
# real `qᵛs(T, p)` (Bolton 1980 saturation vapor pressure plus
# hydrostatic pressure integration), while the in-script `qᵛ_wk(z)`
# above uses the simplified form `qᵛ_max · RH(z)` for brevity. The
